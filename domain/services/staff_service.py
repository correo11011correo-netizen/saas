...
class StaffService:
    ...
    def define_business_term(
        self, tenant_id: UUID, def_type: str, key: str, label: str
    ):
    ...
    def set_permission(self, employee_id: UUID, permission_key: str, granted: bool):
        ...
        if not definition or definition.def_type != "permission":
            raise ValueError(
                f"Permission {permission_key} not defined for this business"
            )

        # 2. Asignar permiso
        perm_provider = self._get_provider("employee_permissions")
        permission = EmployeePermission(
            employee_id=employee_id, permission_key=permission_key, granted=granted
        )

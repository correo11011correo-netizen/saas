...
class BillingCommandHandler:
    ...
    def set_plan(
        ...
        try:
            ...
            session.commit()
            return ServiceResponse.success_res(message=f"Tenant upgraded to {plan_id} plan successfully.")
    ...
    def update_status(
        ...
        try:
            session.execute(
                text("UPDATE tenant_subscriptions SET subscription_status = :status WHERE tenant_id = :tid"),
    ...
    def extend_subscription(
        ...
        try:
            session.execute(
                text("UPDATE tenant_subscriptions SET end_date = end_date + INTERVAL ':days days' WHERE tenant_id = :tid"),

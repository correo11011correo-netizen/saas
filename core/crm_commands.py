...
class CRMCommandHandler:
    ...
    def create_or_update_customer(
        ...
        try:
            ...
            customer = (
                session.execute(
                    text(
                        "SELECT id FROM customers WHERE tenant_id = :tid AND phone_number = :phone"
                    ),
            ...
            if customer:
                ...
                if updates:
                    session.execute(
                        text(
                            f"UPDATE customers SET {', '.join(updates)} WHERE tenant_id = :tid AND phone_number = :phone"
                        ),
                        params,
                    )
    ...
    def get_customer_profile(
        ...
        try:
            ...
            customer = (
                session.execute(
                    text(
                        "SELECT * FROM customers WHERE tenant_id = :tid AND phone_number = :phone"
                    ),
                    {"tid": context.tenant_id, "phone": phone_number},
                )
                .mappings()
    ...
    def list_customers(self, session: Session, context: TenantContext) -> ServiceResponse:
        try:
            customers = (
                session.execute(
                    text(
                        "SELECT id, phone_number, full_name, email, created_at FROM customers WHERE tenant_id = :tid ORDER BY created_at DESC"
                    ),
                    {"tid": context.tenant_id},
                )
                .mappings()
            ...
            return ServiceResponse.success_res(
                data=[dict(c) for c in customers], message="Customers list retrieved."
            )
        except Exception as e:
    ...
    def update_customer(
        ...
        return self.create_or_update_customer(
            session, context, phone_number, full_name, email, metadata
        )

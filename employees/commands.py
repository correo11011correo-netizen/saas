...
class EmployeeCommandHandler:
    ...
    def define_term(
        ...
        try:
            if def_type not in ['permission', 'goal_type', 'task']:
                return ServiceResponse.error_res("Invalid type. Must be 'permission', 'goal_type', or 'task'.", "INVALID_TYPE")
            ...
            session.commit()
            return ServiceResponse.success_res(message=f"Term {def_label} ({def_type}) defined successfully.")
    ...
    @command(
        ...
        params_model={"name": "string", "role": "string", "type": "string", "user_id": "string", "bot_profile_id": "string"},
    )
    def create_employee(
        self, 
        session: Session, 
        ...
        try:
            if type not in ['human', 'bot']:
                return ServiceResponse.error_res("Invalid type. Must be 'human' or 'bot'.", "INVALID_TYPE")
    ...
    def set_permission(
        ...
        try:
            ...
            tenant_id = session.execute(
                text("SELECT tenant_id FROM employees WHERE id = :eid"), {"eid": uuid.UUID(employee_id)}
            ).scalar()
            ...
            if not exists:
                return ServiceResponse.error_res(f"Permission '{permission_key}' is not defined for this business. Use 'staff.define_business_term' first.", "UNDEFINED_PERMISSION")
            ...
            session.commit()
            return ServiceResponse.success_res(message=f"Permission {permission_key} updated for employee.")
        except Exception as e:
            session.rollback()
    ...
    @command(
        ...
        params_model={"employee_id": "string", "goal_type": "string", "target": "float", "start_date": "string", "end_date": "string"},
    )
    def set_goal(
        self, 
        session: Session, 
        ...
        try:
            ...
            if not exists:
                return ServiceResponse.error_res(f"Goal type '{goal_type}' is not defined for this business. Use 'staff.define_business_term' first.", "UNDEFINED_GOAL")
    ...
    def get_report(self, session: Session, context: TenantContext) -> ServiceResponse:
        try:
            ...
            return ServiceResponse.success_res(data=report, message="Staff performance report retrieved.")
        except Exception as e:
            session.rollback()
